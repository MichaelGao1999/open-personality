...
用法:
    python init-skeleton.py [--with-knowledge] [--force] [--target TARGET_DIR]
"""
...
RAW_BASE = "https://raw.githubusercontent.com/MichaelGao1999/agent-coding-skeleton/master"
...
# 母库经验文件（--with-knowledge 时从母库根目录拉取，覆盖 starter/ 的空模板）
KNOWLEDGE_FILES = [
    "lessons-learned.md",
    "troubleshooting.md",
    "ADR.md",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="从 GitHub 拉取 agent-coding-skeleton 骨架基础设施"
    )
    parser.add_argument(
        "--with-knowledge",
        action="store_true",
        help="同时拉取母库经验（lessons-learned.md / troubleshooting.md / ADR.md）",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    if args.with_knowledge:
        files_to_fetch = [f for f in files_to_fetch if f not in KNOWLEDGE_FILES]
