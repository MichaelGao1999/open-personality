...
def test_high_E_extraversion(inference):
    scores = ScoringResult(raw_scores={"O": 50, "C": 50, "E": 80, "A": 50, "N": 30}, t_scores={}, facet_scores={})
    result = inference.infer(scores)
...
def test_low_E_introversion(inference):
    scores = ScoringResult(raw_scores={"O": 50, "C": 50, "E": 20, "A": 50, "N": 50}, t_scores={}, facet_scores={})
    result = inference.infer(scores)
    ei_dim = [d for d in result.dimensions if d.label_a == "E"][0]
...
def test_high_O_intuition(inference):
    scores = ScoringResult(raw_scores={"O": 80, "C": 50, "E": 50, "A": 50, "N": 50}, t_scores={}, facet_scores={})
    result = inference.infer(scores)
...
def test_high_A_feeling(inference):
    scores = ScoringResult(raw_scores={"O": 50, "C": 50, "E": 50, "A": 80, "N": 50}, t_scores={}, facet_scores={})
    result = inference.infer(scores)
...
def test_high_C_conscientiousness(inference):
    scores = ScoringResult(raw_scores={"O": 50, "C": 80, "E": 50, "A": 50, "N": 50}, t_scores={}, facet_scores={})
    result = inference.infer(scores)
