...
def test_get_items_standard_zh():
    response = client.get("/questionnaires/items?mode=standard&lang=zh")
...
def test_get_items_standard_en():
    response = client.get("/questionnaires/items?mode=standard&lang=en")
    assert response.status_code == 200
    data = response.json()
    assert len(data["items"]) == 120
...
def test_get_items_invalid_mode():
    response = client.get("/questionnaires/items?mode=invalid&lang=zh")
    assert response.status_code == 422


def test_get_items_invalid_lang():
    response = client.get("/questionnaires/items?mode=standard&lang=fr")
    assert response.status_code == 422


def test_submit_valid():
    answers = [{"item_id": f"ipip_{i:03d}", "value": 3} for i in range(1, 121)]
    response = client.post("/questionnaires/submit", json={
...
def test_get_report_by_token():
    answers = [{"item_id": f"ipip_{i:03d}", "value": 3} for i in range(1, 121)]
    submit_resp = client.post("/questionnaires/submit", json={
        "mode": "standard", "lang": "zh", "answers": answers,
    })
...
def test_get_report_not_found():
    response = client.get("/report/nonexist")
    assert response.status_code == 404


def test_get_i18n_zh():
    response = client.get("/i18n/zh")
    assert response.status_code == 200
